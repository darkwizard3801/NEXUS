        ...
        {loading ? (
          ...
          data.filter((product) => !product.disabled).map((product, index) => (
              <Link
                ...
                key={index}
                className='relative w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] h-48 bg-white rounded-sm shadow overflow-hidden group'
              >
                <img
                ...
                />
                <div className='absolute top-0 right-0 h-full w-1/2 bg-white shadow-lg transition-transform duration-300 transform translate-x-[100%] group-hover:translate-x-0 flex flex-col justify-center p-4'>
                  ...
                  <p className='text-black-600 font-medium'>{displayINRCurrency(product?.price)}</p>
                  <button
                    className='text-sm bg-red-600 hover:bg-red-700 text-white px-3 py-0.5 rounded-full w-fit'
                    onClick={(e) => {
