        ...
        <div className='min-h-[calc(100vh-120px)] flex flex-col md:flex-row'> {/* Updated flex direction */}
            <aside className={`bg-white min-h-full w-full max-w-60 customShadow ${isOpen ? 'block' : 'hidden'} md:block`}> {/* Conditional rendering */}
                ...
                <nav className='grid p-4'>
                        ...
                        <Link to={"orders"} className='px-2 py-1 rounded-2xl hover:bg-blue-600 hover:text-white hover:rounded-2xl transition duration-300 ease-in-out transform hover:scale-105'>My Orders</Link>
                        ...
                        <Link to={"my-events"} className='px-2 py-1 rounded-2xl hover:bg-blue-600 hover:text-white hover:rounded-2xl transition duration-300 ease-in-out transform hover:scale-105'>My Events</Link>
                    </nav>
                </div>
            </aside>
